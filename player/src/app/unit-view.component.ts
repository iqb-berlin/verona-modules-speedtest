...
@Component({
  ...
  template: `
      <div class="outer-wrapper">
        @if (unit.instructionText) {<p class="instruction-text">{{unit.instructionText}}</p>}
            ...
            <div class="question">
                ...
                @if (unit.questionType === 'audio') {
                    <audio #audioElement [src]=question.src
                           (play)="audioActive = true" (ended)="audioActive = false"></audio>
                    <button class="audio-button" [class.in-progress]="audioActive"
                            [disabled]="audioActive" (click)="audioElement.play();">
...
<!--      <div class="outer-wrapper" [class.numbers]="unit.answerType === 'number'"-->
<!--           [class.instruction]="unit.instructionText">-->
<!--          @if (unit.instructionText) {<p class="instruction-text">{{unit.instructionText}}</p>}-->
<!--          <div class="wrapper" [style.flex-direction]="layout"-->
<!--               [ngClass]="{'column': layout == 'column',-->
<!--                           'row': layout == 'row',-->
<!--                           'image': unit.questionType === 'image'}">-->
<!--              <div class="question" [class.image-label]="question.src && question.text">-->
<!--                  @if (unit.questionType === 'image') {-->
<!--                      <img *ngIf="question.src" [src]="question.src" [alt]="question.text">-->
<!--                  }-->
<!--                  @if (unit.questionType === 'audio') {-->
<!--                      <audio #audioElement [src]=question.src-->
<!--                             (play)="audioActive = true" (ended)="audioActive = false"></audio>-->
<!--                      <button class="audio-button" [class.active]="audioActive"-->
<!--                              [disabled]="audioActive" (click)="audioElement.play();">-->
<!--                          <svg id="Layer_1" xmlns="http://www.w3.org/2000/svg"-->
<!--                               xmlns:xlink="http://www.w3.org/1999/xlink"-->
<!--                               width="180px" height="180px" viewBox="0 0 64 64" xml:space="preserve">-->
...
<!--              </div>-->
<!--              @if (unit.answerType !== 'number') {-->
<!--                  <div class="answers" [ngClass]="{'image': unit.answerType === 'image'}">-->
<!--                      @for (answer of question.answers; let answerIndex = $index; track answer) {-->
<!--                          @if (unit.answerType === 'text') {-->
<!--                              <button mat-raised-button [style.background-color]="buttonColor"-->
...
<!--                              </button>-->
<!--                          } @else if (unit.answerType === 'image') {-->
<!--                              <input type="image" [src]="answer" (click)="responseGiven.emit(answerIndex)">-->
<!--                          }-->
<!--                      }-->
<!--                  </div>-->
<!--              } @else {-->
<!--                  <div class="number-result-boxes">-->
<!--                      @for (number of numberAnswer; track $index) {-->
<!--                          <div class="number-result-box" [class.active]="$index === activeNumberIndex">-->
<!--                              {{ number }}-->
<!--                          </div>-->
...
<!--              <div class="unit-nav-next">-->
<!--                  <span class="button-text">Hier geht’s weiter.</span>-->
<!--                  <span class="svg-container" (click)="responseGiven.emit($any(numberAnswer))">-->
<!--                    <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 100 100">-->
<!--                        <circle cx="50%" cy="50%" r="50" fill="#b3fe5b"/>-->
<!--                        <path d="M 45 35 L 60 50 L 45 65" stroke="black" stroke-width="5" fill="none" />-->
<!--                    </svg>-->
